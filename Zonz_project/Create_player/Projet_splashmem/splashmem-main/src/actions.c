...
static uint8_t cpt_par_appel = 0;
...
void actions_do(t_player* p_player, enum action act_id)
{
    ...
    /******************************** DASH ***************************/
    case ACTION_DASH_L:
        if (p_player->credits >= NB_CREDIT_DASH)
        {
            for (i = 0; i < 8; i++)
            {
                if (mem_player <= 0)
            ...
            p_player->x       = mem_player;
            p_player->credits = p_player->credits - NB_CREDIT_DASH;
        }
        break;
    case ACTION_DASH_R:
        if (p_player->credits >= NB_CREDIT_DASH)
    ...
    case ACTION_TELEPORT_L:
        if (p_player->credits >= NB_CREDIT_TELEP)
        {
            if (mem_player < 8) //
            {
            ...
            world_paint_spot(mem_player, mem_player_y, id_player);
            p_player->x       = mem_player;
            p_player->credits = p_player->credits - NB_CREDIT_TELEP;
        }
        break;
    ...
    case ACTION_MOVE_UP_RIGHT:
        if (mem_player < MAP_SIZE - 1 && mem_player_y > 0)
        {
            world_paint_spot(mem_player + 1, mem_player_y - 1, id_player);
            p_player->x = mem_player + 1;
            p_player->y = mem_player_y - 1;
            p_player->credits--;
        }
    ...
    case ACTION_MOVE_UP_LEFT:
        if (mem_player > 0 && mem_player_y > 0)
        {
            world_paint_spot(mem_player - 1, mem_player_y - 1, id_player);
            p_player->x = mem_player - 1;
            p_player->y = mem_player_y - 1;
    ...
    case ACTION_MOVE_DOWN_RIGHT:
        if (mem_player < MAP_SIZE - 1 && mem_player_y < MAP_SIZE - 1)
        {
            world_paint_spot(mem_player + 1, mem_player_y + 1, id_player);
            p_player->x = mem_player + 1;
            p_player->y = mem_player_y + 1;
    ...
    case ACTION_MOVE_DOWN_LEFT:
        if (mem_player > 0 && mem_player_y < MAP_SIZE - 1)
        {
            world_paint_spot(mem_player - 1, mem_player_y + 1, id_player);
            p_player->x = mem_player - 1;
            p_player->y = mem_player_y + 1;
